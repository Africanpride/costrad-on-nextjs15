...
export function SignUpComponent() {
	...
	return (
		<div className="min-h-dvh sm:pt-0 pt-12 ">
			<Card className="z-50 rounded-md rounded-t-none max-w-md border-none shadow-none ">
							...
							onClick={async () => {
								...
								const signUpPromise = signUp.email({
									...
									if (response?.data?.user?.emailVerified === false) {
										...
										setSignUpResponse("Email verification required. Redirecting ...");
									}
				...
				<CardFooter className=" flex flex-col">
					<div className="text-balance text-center text-sm text-muted-foreground [&_a]:underline [&_a]:underline-offset-4 hover:[&_a]:text-primary">
						Don't have an account? Rather <Link href={'/auth/sign-in'} className="font-bold">Login</Link>.
					</div>
